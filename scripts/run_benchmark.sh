...
./$engine.sh --quiet $* &> $engine.log &
   echo -n "Waiting for mongod ($engine) to become ready ..."
    while [ $(echo 'db.isMaster()' | /mnt/storage/PERF-15/fipar/percona-server-mongodb-3.0.10-1.5/bin/mongo 2>/dev/null|grep ismaster|grep -c true) -eq 0 ]; do
       sleep 0.3
    done
    echo " Done"
popd
	...
	for extra in "--profile=0 --slowms=100000" "--profile=2 --slowms=0"; do
	...
	echo "Starting benchmark"
		...
		--oltp-inserts=1 run 2>&1 | tee sysbench-$engine-$size-$threads-$workload-profile$profile.txt
