...
# sysbench-ssd-inmemory-uniform-48-updates.txt
echo "engine,"$(env _ONLYHEADER=1 csv_from_sysbench.sh ../raw/sysbench-ssd-inmemory-uniform-48-updates.txt updates 8000000 48) > ../data-updates.csv
for engine in inmemory wt; do
    env _NOHEADER=1 csv_from_sysbench.sh ../raw/sysbench-ssd-$engine-uniform-48-updates.txt updates 8000000 48 | while read l; do
														     echo $engine,$l
														     done >> ../data-updates.csv
done # for engine in ...
