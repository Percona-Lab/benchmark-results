...
echo "range_size,engine,distribution,$(env _ONLYHEADER=1 csv_from_sysbench.sh ../raw/sysbench-iobound-1000-pareto-psfm32-rocks-60000000-1-ranges_ro.txt psfm 60000000 1)" > ../alldata.csv
	...
	for engine in wt rocks; do
		...
		for f in ../raw/sysbench-$workload-$range_size-$distribution-psfm32-$engine-*; do
		    ...
		    env _NOHEADER=1 csv_from_sysbench.sh $f $workload 2000000 $threads | while read l; do
											    echo "$range_size,$engine,$distribution,$l" >> ../alldata.csv
											    done
		done
	    done
	done
