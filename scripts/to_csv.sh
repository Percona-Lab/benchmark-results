...
echo "version,memory,engine,distribution,$(env _ONLYHEADER=1 csv_from_sysbench.sh ../raw/sysbench-PERF22_20-uniform-upstream-3.2-wt-60000000-32-oltp.txt psfm 60000000 1)" > ../alldata.csv
	...
	for engine in wt rocks ft; do
		...
		for version in upstream-3.2 psmdb-3.2; do
			...
			env _NOHEADER=1 csv_from_sysbench.sh $f $workload 2000000 $threads | while read l; do
												echo "$version,$memory,$engine,$distribution,$l" >> ../alldata.csv
												done
		    done
		done
	    done
