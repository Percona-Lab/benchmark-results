...
m+geom_bar(stat = "identity",position = "dodge",aes(fill = as.factor(variable)))+geom_hline(yintercept=1)+geom_text(aes(label=format(value, digits=2, nsmall=2), 
                hjust=1), 
            position = position_dodge(width=0.9),size=3,angle = 90)+labs(x="Threads",y="relative performance",title="sysbench OLTP RO, ProxySQL overhead 5.7")+scale_fill_tableau(name="Version")+scale_y_continuous(breaks=seq(0, 1.5, 0.1),labels = scales::comma)+theme(axis.text.x  = element_text(angle=45, vjust=0.5))+tm+theme(legend.position="bottom")
