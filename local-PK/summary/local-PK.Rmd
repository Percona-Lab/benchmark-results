...
#data = read.table("201603-mysql55-56-57-RO/local-PK/summary.csv",sep=",",header=F)
...
m+geom_line()+labs(x="Threads",y="Throughput,tps",title="sysbench read-only")+scale_color_tableau(name="Version")
```
...
* mysql56sc.no-perf-schema-compiled.ibpi32 - PERFORMANCE_SCHEMA was not compiled

```{r, echo=FALSE, warning=FALSE,fig.width=10, fig.height=8}
ss=subset(aggdata,(Group.2<=1000)&(Group.1 %in% c("mysql55","mysql56.itc56.ibpi32.psOFF","mysql56.ibpi32","mysql56sc.no-perf-schema-compiled.ibpi32") ))
#ss=subset(aggdata)

m <- ggplot(ss, 
            aes(x=as.factor(Group.2),y=as.numeric(V2),color=as.factor(Group.1),group=Group.1))
m+geom_line()+labs(x="Threads",y="Throughput,tps",title="sysbench read-only, Performance Schema overhead in 5.6")+scale_color_tableau(name="Version")+scale_y_continuous(labels = scales::comma)
```

## Performance Schema overhead in 5.7

* psOFF - MySQL 5.7 started with --performance-schema=OFF
* mysql57sc.no-perf-schema-compiled.ibpi32 - PERFORMANCE_SCHEMA was not compiled

```{r, echo=FALSE, warning=FALSE,fig.width=10, fig.height=8}
ss=subset(aggdata,(Group.2<=1000)&(Group.1 %in% c("mysql55","mysql57.ibpi32","mysql57.ibpi32.psOFF","mysql57sc.no-perf-schema-compiled.ibpi32") ))
#ss=subset(aggdata)
...
* psOFF - MySQL 5.7 started with --performance-schema=OFF
* mysql57sc.no-perf-schema-compiled.ibpi32 - PERFORMANCE_SCHEMA was not compiled
* mysql57.ibpi32.imON - MySQL 5.7 with INNODB_METRICS enabled (all)

```{r innodbmetrics, echo=FALSE, fig.height=8, fig.width=10, warning=FALSE}
ss=subset(aggdata,(Group.2<=1000)&(Group.1 %in% c("mysql57.ibpi32","mysql57.ibpi32.imON") ))
#ss=subset(aggdata)
...
wide_format <- dcast(subset(Res, is.element(aggdata.Group.1,c("mysql55","mysql56.ibpi32","mysql57.ibpi32","mysql56.itc56.ibpi32.psOFF","mysql57.ibpi32.psOFF") )), aggdata.Group.2 ~ aggdata.Group.1)
