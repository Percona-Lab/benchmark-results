...
```{r, echo=FALSE, warning=FALSE,fig.width=10, fig.height=8}
ss=subset(aggdata,(Group.2<=1000)&(Group.1 %in% c("mysql57.bpi32","mysql57.psOFF","mysql57.tiRC.psALL_ON","mysql57.tiRC.no-perf-schema-compiled","mysql57.PS-CONS-ALL") ))
#ss=subset(aggdata)
...
wide_format <- dcast(subset(Res, is.element(aggdata.Group.1,c("mysql57.bpi32","mysql57.psOFF","mysql57.tiRC.psALL_ON","mysql57.tiRC.no-perf-schema-compiled","mysql57.PS-CONS-ALL","mysql57.PS-CONS-ALL") )), aggdata.Group.2 ~ aggdata.Group.1)
...
#ss=subset(aggdata)
wide_format$`psOFF` = wide_format$`mysql57.psOFF`/wide_format$`mysql57.bpi32`
wide_format$`no-perf-schema-compiled` = wide_format$`mysql57.tiRC.no-perf-schema-compiled`/wide_format$`mysql57.bpi32`
wide_format$`psALL_ON` = wide_format$`mysql57.tiRC.psALL_ON`/wide_format$`mysql57.bpi32`
wide_format$`ps_CONSUMER_ALL_ON` = wide_format$`mysql57.PS-CONS-ALL`/wide_format$`mysql57.bpi32`
...
dat_m <- melt(subset(wide_format,!is.na(psALL_ON)), id.vars = "aggdata.Group.2", measure.vars=c("psOFF","no-perf-schema-compiled","psALL_ON","ps_CONSUMER_ALL_ON"))
...
m+geom_bar(stat = "identity",position = "dodge",aes(fill = as.factor(variable)))+geom_hline(yintercept=1)+geom_text(aes(label=format(value, digits=2, nsmall=2), 
                hjust=1), 
            position = position_dodge(width=0.9),size=3,angle = 90)+labs(x="Threads",y="relative performance",title="sysbench read-only, Performance Schema overhead in 5.7")+scale_fill_tableau(name="Version")+scale_y_continuous(breaks=seq(0, 1.5, 0.1),labels = scales::comma)+theme(axis.text.x  = element_text(angle=45, vjust=0.5))+tm
```
...
ss=subset(aggdata,(Group.2<=1000)&(Group.1 %in% c("mysql57.short","mysql57.imON") ))
#ss=subset(aggdata)

m <- ggplot(ss, 
            aes(x=as.factor(Group.2),y=as.numeric(V2),color=as.factor(Group.1),group=Group.1))
m+geom_line()+labs(x="Threads",y="Throughput,tps",title="sysbench read-only, INNODB_METRICS overhead in 5.7")+scale_color_tableau(name="Version")+scale_y_continuous(labels = scales::comma)
```
