...
```{r cpu, fig.width=36, fig.height=16, warning=F, echo=F}

for (y in types) {
	for (t in nothreads) {
		for (p in noparthreads) {
			dataMelted <- melt(subset(vmstatParallel, type==y & disk == "ssd" & threads == t & parallel_threads == p),
			id=c("time"), measure=c("us","sy","wa"))
			attach(dataMelted)
			p = ggplot(dataMelted, aes(x=time, y=value, color=variable)) +
			geom_jitter(size=8,width=0.2,height=0.2) + theme_bw() +
			labs(x="time (secs)", y="cpu time", title=paste("Backup CPU usage - ssd, ", t, " threads, ", p, " --parallel threads")) +
			expand_limits(x=0,y=0) +
			scale_y_continuous(breaks=extended_range_breaks()(rbind(0,value))) +
			theme(panel.grid = element_line(colour="#010101",size=1), panel.background = element_rect(colour="#000000"), plot.background = element_rect(colour="#000000"), strip.background = element_rect(colour="#010101"), text=element_text(size=50), strip.text=element_text(size=60))
			print(p)
            detach(dataMelted)
		}
