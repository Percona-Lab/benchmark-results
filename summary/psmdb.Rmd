...
for (d in disks) {
    subdata <- subset(data_pareto, disk == d & range_size <= 10000)
    ...
    subdata <- subset(data_uniform, disk == d & range_size <= 10000)
...
for (d in disks) {
	for (rs in ranges) {
	data_wt_range <- subset(data_wt, range_size == rs & disk == d)
	attach(data_wt_range)
	print(ggplot(data_wt_range, aes(x=ts, y=tps, colour=as.factor(workload))) + geom_jitter(size=4) + scale_colour_discrete(name = "workload") + facet_grid(distribution ~ user_provided_threads, labeller=label_custom) + xlab("Time in seconds (10 sec interval)") + ylab("Throughput (read/write ops per second)") + ggtitle(paste("Percona Server for MongoDB / throughput per threads (wt, rs ",rs,", ",d,")")) + scale_y_continuous(breaks = extended_range_breaks()(rbind(0,data_wt_range$tps)))  + scale_x_discrete(breaks = c(600)) + expand_limits(x=0, y=0) + theme_bw() + theme(panel.grid = element_line(colour="#010101",size=1), panel.background = element_rect(colour="#000000"), plot.background = element_rect(colour="#000000"), strip.background = element_rect(colour="#010101"), text=element_text(size=50), strip.text=element_text(size=60)))
	detach(data_wt_range)
	}	
}
...
for (d in disks) {
	for (rs in ranges) {
	data_rocks_range <- subset(data_rocks, range_size == rs & disk == d)
	attach(data_rocks_range)
	print(ggplot(data_rocks_range, aes(x=ts, y=tps, colour=as.factor(workload))) + geom_jitter(size=4) + scale_colour_discrete(name = "workload") + facet_grid(distribution ~ user_provided_threads, labeller=label_custom) + xlab("Time in seconds (10 sec interval)") + ylab("Throughput (read/write ops per second)") + ggtitle(paste("Percona Server for MongoDB / throughput per threads (rocks, rs ",rs,", ",d,")")) + scale_y_continuous(breaks = extended_range_breaks()(rbind(0,data_rocks_range$tps)))  + scale_x_discrete(breaks = c(600)) + expand_limits(x=0, y=0) + theme_bw() + theme(panel.grid = element_line(colour="#010101",size=1), panel.background = element_rect(colour="#000000"), plot.background = element_rect(colour="#000000"), strip.background = element_rect(colour="#010101"), text=element_text(size=50), strip.text=element_text(size=60)))
	detach(data_rocks_range)
	}	
}
