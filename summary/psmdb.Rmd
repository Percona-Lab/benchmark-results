...
aggdata_pareto <- aggregate(data_pareto$tps, by=list(data_pareto$engine, data_pareto$user_provided_threads, data_pareto$workload), FUN=function(x) quantile(x, probs=0.99))
aggdata_uniform <- aggregate(data_uniform$tps, by=list(data_uniform$engine, data_uniform$user_provided_threads, data_uniform$workload), FUN=function(x) quantile(x, probs=0.99))
...
m+ facet_grid(. ~ Group.3) +geom_line()+geom_point(size=4) + labs(x="Threads",y="Throughput,tps",title="sysbench Ranges - Pareto")+scale_color_tableau(name="Engine")+scale_y_continuous(breaks=extended_range_breaks()(rbind(0,data_pareto$tps)),labels = scales::comma)+theme(axis.text.x  = element_text(angle=45, vjust=0.5))+theme_bw() + theme(panel.grid = element_line(colour="#010101",size=1), panel.background = element_rect(colour="#000000"), plot.background = element_rect(colour="#000000"), strip.background = element_rect(colour="#010101"), text=element_text(size=50), strip.text=element_text(size=60))

m <- ggplot(aggdata_uniform, aes(x=as.factor(Group.2),y=as.numeric(aggdata_uniform$x),color=as.factor(Group.1),group=Group.1))
m+geom_line()+geom_point(size=4) + facet_grid(. ~ Group.3) +labs(x="Threads",y="Throughput,tps",title="sysbench Ranges - Uniform")+scale_color_tableau(name="Engine")+scale_y_continuous(breaks=extended_range_breaks()(rbind(0,data_uniform$tps)),labels = scales::comma)+theme(axis.text.x  = element_text(angle=45, vjust=0.5))+theme_bw() + theme(panel.grid = element_line(colour="#010101",size=1), panel.background = element_rect(colour="#000000"), plot.background = element_rect(colour="#000000"), strip.background = element_rect(colour="#010101"), text=element_text(size=50), strip.text=element_text(size=60))
