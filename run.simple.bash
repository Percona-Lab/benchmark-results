...
javac -cp $CLASSPATH:$PWD/src:$PWD src/jmongosysbenchload.java
javac -cp $CLASSPATH:$PWD/src:$PWD src/jmongosysbenchexecute.java
...
if [[ $DOLOAD = "yes" ]]; then
    ...
    export LOG_NAME=${OUTDIR}mongoSysbenchLoad-${NUM_COLLECTIONS}-${NUM_DOCUMENTS_PER_COLLECTION}-${NUM_LOADER_THREADS}.txt
...
if [[ $DOQUERY = "yes" ]]; then
    ...
    export LOG_NAME=${OUTDIR}mongoSysbenchExecute-${NUM_COLLECTIONS}-${NUM_DOCUMENTS_PER_COLLECTION}-${NUM_WRITER_THREADS}.txt
    export BENCHMARK_TSV=${LOG_NAME}.tsv
